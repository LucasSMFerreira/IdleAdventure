...
func _ready() -> void:
	...
	UIFactory.action(map_panel, "◀", Vector2(218, 8), Vector2(34, 25), _change_floor.bind(-1))
	...
	UIFactory.action(map_panel, "▶", Vector2(364, 8), Vector2(34, 25), _change_floor.bind(1))
	...
	for stage: int in range(1, 11):
		...
		button.pressed.connect(_start.bind(stage))
		map_panel.add_child(button)
		nodes.append(button)
	status = UIFactory.text(map_panel, "", Vector2(25, 205), Vector2(565, 52), 11, DarkTheme.TEXT)
	status.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	UIFactory.action(map_panel, "Entrar na fase atual", Vector2(210, 256), Vector2(195, 30), _start_current, DarkTheme.GOLD)
...
func refresh() -> void:
	...
	var current_stage := (EstadoJogo.maior_fase_liberada - 1) % 10 + 1 if shown_floor == _last_floor() else 10
	for index: int in range(nodes.size()):
		var stage := index + 1
		var unlocked := (shown_floor - 1) * 10 + stage <= EstadoJogo.maior_fase_liberada
		var button := nodes[index]
		button.disabled = not unlocked
		var border := DarkTheme.CRIMSON if stage == 10 else (Color("22c55e") if stage == current_stage and shown_floor == _last_floor() else Color("e5e7eb"))
		button.add_theme_stylebox_override("normal", DarkTheme.panel(Color("241d19"), border, 3))
		button.add_theme_color_override("font_color", DarkTheme.GOLD if stage == 10 else DarkTheme.TEXT)
	status.text = "%s\nLiberado até [%d-%d] • Nível %d • %d Gold" % [DETAILS[EstadoJogo.difficulty_selected], _last_floor(), (EstadoJogo.maior_fase_liberada - 1) % 10 + 1, EstadoJogo.nivel, EstadoJogo.gold]
...
func _start(stage: int) -> void:
	if not EstadoJogo.selecionar_fase(shown_floor, stage): return
...
func _start_current() -> void:
	shown_floor = _last_floor()
	_start((EstadoJogo.maior_fase_liberada - 1) % 10 + 1)

...
@onready var actor: Player = get_node(player_path) as Player
...
var fase: int = 1
var tempo: float = 0.0
var velocidade_visual: float = 0.0
const TONS_CEU = [
...
func configurar(novo_andar: int, nova_fase: int) -> void:
    andar = novo_andar
    fase = nova_fase
    queue_redraw()
...
func _process(delta: float) -> void:
    velocidade_visual = move_toward(velocidade_visual, actor.velocity.x, delta * 420.0)
    tempo += delta * velocidade_visual / maxf(actor.stats.move_speed, 1.0)
    queue_redraw()

func _draw() -> void:
    var cor_ceu: Color = TONS_CEU[(andar - 1) % TONS_CEU.size()]
    var cor_fundo: Color = cor_ceu.lerp(Color(0.95, 0.95, 0.95), 0.5)
    var passo_ceu: float = fmod(tempo * 8.0, 128.0)
    var passo_fundo: float = fmod(tempo * 2.0, 160.0)
    for coluna in range(9):
        var quadro_ceu: int = (coluna + andar) % 2
